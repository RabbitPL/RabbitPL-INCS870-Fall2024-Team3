...
# 分类和调参函数
def node_classification(args, embeddings, param_name="n_estimators", param_values=None):
    ...
    for param_value in param_values:
        ...
        model = LGBMClassifier(boosting_type='gbdt', **{param_name: param_value}, num_leaves=31, learning_rate=0.1,
                               random_state=args.seed)
...
def run_trans2vec(args):
    ...
    # 调参的参数名和参数范围
    param_name = "n_estimators"  # 修改为 n_estimators
    param_values = np.arange(350, 700, 50)  # 需要调整的参数值范围，可以修改为其他值
