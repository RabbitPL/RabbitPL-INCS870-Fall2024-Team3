...
# 分类和调参函数
def node_classification(args, embeddings, param_name="learning_rate", param_values=None):
    ...
    for param_value in param_values:
        ...
        model = LGBMClassifier(boosting_type='gbdt', learning_rate=param_value, num_leaves=31, n_estimators=100,
                               random_state=args.seed)
...
def run_trans2vec(args):
    ...
    # 调参的参数名和参数范围
    param_name = "learning_rate"  # 调整为 'learning_rate'
    param_values = np.arange(0.15, 0.30, 0.01)  # 学习率从 0.01 到 0.3，步长为 0.02
